...
VIDEOS = [
    ...
    # מתוך 10 תיקיות תואמות, 2 הכילו תמונה (לא וידאו): "תמונה 8.9 חדש", "תמונה 2 8.9 חדש" -
    # אלה נכללות למטה כמודעות תמונה (media_type="image"), ראו find_media_and_text_in_folder
    # ב-drive_videos.py שמזהה את סוג המדיה אוטומטית לפי מה שקיים בפועל בתיקייה.
    # גם "תיקייה ללא שם" וקובץ בודד בשורש התיקייה לא תאמו את התבנית המבוקשת
    # (תאריך+חדש) והושמטו.
    ...
    # שתי תיקיות התמונה מהסבב השני - media_type="image" גורם ל-campaign_launch.py
    # להעלות דרך image_upload.py (/adimages) וליצור קריאטיב עם link_data במקום video_data.
    {"match": "תמונה 8.9", "ad_set_name": "מה שבפנים יוצר את מה שבחוץ",
     "message": "מה שבפנים - יוצר את מה שבחוץ. המחשבות שלך משפיעות על הפרשנות, הפרשנות על ההחלטות, וההחלטות בונות את המציאות שלך. עוד תוכן כזה - עקבו באינסטגרם",
     "title": "מה שבפנים יוצר את מה שבחוץ",
     "description": "השינוי האמיתי מתחיל הרבה לפני הפעולה",
     "category": "personal_dev_psych",
     "media_type": "image"},
    {"match": "תמונה 2 8.9", "ad_set_name": "אתה רואה את המציאות דרך התודעה שלך",
     "message": "אתה לא רואה את המציאות כמו שהיא - אתה רואה אותה דרך התודעה שלך. תשנה את הפרשנות ותתחיל לשנות את התוצאה. עוד תוכן כזה - עקבו באינסטגרם",
     "title": "תשנה פרשנות, תשנה תוצאה",
     "description": "המציאות עוברת דרך התודעה שלך",
     "category": "personal_dev_psych",
     "media_type": "image"},
]
